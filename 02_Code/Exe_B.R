...
df <- merge(Municipality[Municipality$NAME_1 %in% Jesuit, ], ds, by = c("COUNTRY", "NAME_1", "NAME_2"), all = TRUE)
...
plot_01 <- ggplot() +
  ...
  geom_sf(data = Region[Region$NAME_1 %in% Jesuit, ], aes(geometry = geometry, size = 1.2), color = "black", fill = NA) +
  ...
  geom_label(data = Region[Region$NAME_1 %in% Jesuit, ], aes(x = X, y = Y, label = NAME_1),
                   size = 3, color = "black",fill = "white", fontface = "bold",
                   label.padding = unit(0.5, "lines")) +
  # geom_label_repel(data = Region[Region$NAME_1 %in% Jesuit, ], aes(x = X, y = Y, label = NAME_1),
  #                  size = 3, color = "black",fill = "white", fontface = "bold",
  #                  arrow = arrow(angle = 25, length = unit(1, "mm"), ends = "first", type = "closed"),
...
plot_03 <- ggplot() +
  ...
  theme(plot.title = element_text(size = 12, face = "bold"),
        plot.subtitle = element_text(size = 11, face = "italic"),
        legend.position = "inside",
        legend.position.inside = c(0.10, 0.20),
        legend.box.just = "left",
        legend.box.background = element_rect(color = "black", linetype = "solid")) +
  geom_label(data = Region[Region$NAME_1 %in% Jesuit, ], aes(x = X, y = Y, label = NAME_1),
             size = 3, color = "black",fill = "white", fontface = "bold",
...
df_tm <- merge(Municipality[Municipality$NAME_1 %in% Jesuit, ], ds, by = c("COUNTRY", "NAME_1", "NAME_2"), y.all = TRUE)
plot_04 <- tm_shape(df_tm) + 
...
breaks <- seq(min(df$distmiss), max(df$distmiss), length.out = 10)
labels <- sprintf("%.1f to %.1f", breaks, c(breaks[-1], max(df$distmiss)))

Dist_1 <- ggplot() +
  geom_sf(data = df, aes(geometry = geometry, fill = distmiss), color = "gray") +
  geom_sf(data = Region, aes(geometry = geometry), color = "#737373", fill = NA) +
  geom_sf(data = Country, aes(geometry = geometry), color = "black", fill = NA) +
...
breaks_log <- seq(min(df$distmiss_log), max(df$distmiss_log), length.out = 10)
labels_log <- sprintf("%.1f to %.1f", breaks_log, c(breaks_log[-1], max(df$distmiss_log)))

Dist_2 <- ggplot() +
  geom_sf(data = df, aes(geometry = geometry, fill = distmiss_log), color = "gray") +
  geom_sf(data = Region, aes(geometry = geometry), color = "#737373", fill = NA) +
  geom_sf(data = Country, aes(geometry = geometry), color = "black", fill = NA) +
...
breaks_dec <- seq(min(df$distmiss_dec), max(df$distmiss_dec), length.out = 10)
labels_dec <- sprintf("%.1f to %.1f", breaks_dec, c(breaks_dec[-1], max(df$distmiss_dec)))

Dist_3 <- ggplot() +
  geom_sf(data = df, aes(geometry = geometry, fill = distmiss_dec), color = "gray") +
  geom_sf(data = Region, aes(geometry = geometry), color = "#737373", fill = NA) +
  geom_sf(data = Country, aes(geometry = geometry), color = "black", fill = NA) +
  scale_fill_gradient(name = "Distance",
...
breaks_root <- seq(min(df$distmiss_root), max(df$distmiss_root), length.out = 10)
labels_root <- sprintf("%.1f to %.1f", breaks_root, c(breaks_root[-1], max(df$distmiss_root)))

Dist_4 <- ggplot() +
  geom_sf(data = df, aes(geometry = geometry, fill = distmiss_root), color = "gray") +
  geom_sf(data = Region, aes(geometry = geometry), color = "#737373", fill = NA) +
  geom_sf(data = Country, aes(geometry = geometry), color = "black", fill = NA) +
  scale_fill_gradient(name = "Root Distance:",
                      low = "#800026", high = "#ffffcc",
                      breaks = breaks_root,
                      labels = labels_root) +
  theme_light() +
  scale_size_identity() +
...
model_a <- lm(illiteracy ~ distmiss + lati + longi + corr + ita + mis_pry + mis, data = df)
model_b <- lm(illiteracy ~ distmiss_log + lati + longi + corr + ita + mis_pry + mis, data = df)
model_c <- lm(illiteracy ~ distmiss_dec + lati + longi + corr + ita + mis_pry + mis, data = df)
model_d <- lm(illiteracy ~ distmiss_root + lati + longi + corr + ita + mis_pry + mis, data = df)

## STARGAZER --------------------------------------------------------------------
list1 <- list(model_1, model_2, model_3, model_4, model_5, model_6, model_7, model_8)
stargazer(list1, title = "Baseline Model", digits = 4,  type = "latex")

list2 <- list(conley_1, conley_2, conley_3, conley_4, conley_5, conley_6, conley_7, conley_8)
stargazer(list2, title = "Models with Conley SE", digits = 4, type = "latex")

stargazer(model_a, model_b, model_c, model_d, title = "Models with different distances", digits = 4, type = "latex")

# SAVING -----------------------------------------------------------------------
ggsave("plot_00.png", plot = plot_00, device = "png")
ggsave("plot_01.png", plot = plot_01, device = "png")
ggsave("plot_02.png", plot = plot_02, device = "png")
ggsave("plot_03.png", plot = plot_03, device = "png")
tmap_save(plot_04, filename = "plot_04.png")
ggsave("Dist_01.png", plot = Dist_1, device = "png")
ggsave("Dist_02.png", plot = Dist_2, device = "png")
ggsave("Dist_03.png", plot = Dist_3, device = "png")
ggsave("Dist_04.png", plot = Dist_4, device = "png")
